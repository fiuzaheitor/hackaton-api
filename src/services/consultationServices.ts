...
export const sendMessagesToAllNecessities = async () => {
    ...
    try {
      ...
      const filteredVaccines = vaccines.filter(
        (vaccine: any) => calculateDifference(vaccine.date) <= 7 && !vaccine.isFinished
      );
      ...
      for(const vaccine of filteredVaccines as any[]) {
          try {
            const vaccineCard: any = await VaccineCards.findById(vaccine.vaccineCard);
            const vaccineTemplate: any = await VaccineTemplates.findById(vaccine.vaccineTemplate);
